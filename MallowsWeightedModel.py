...
# Uses back-tracking algorithm, but we only search for phi values. Keep b fixed at 0 .
def unweighted_backtrack(data, num_iter=100, tolerance=0.0005, ground_truth=None, 
    plot_grads=True, bounds=True, b_start=None, phi_start=None):
    num_elements = data[0].shape[0]
...
def main():
    ...
    # # out = mult_starts(all_rankings_data, plot_grads=False)

    # print(out)

    # actual_best_phi, actual_best_b, actual_max_log_like = exhaustive_search(all_rankings_data, plot_log_likes=True)
    # pred_best_b, pred_best_phi, pred_max_log_like = alt_grad_ascent(all_rankings_data, plot_grads=False, ground_truth=GROUND_TRUTH)
    ...
    # Simulated Bimodal Ground-Truth Dataset 
    # WEIGHT_A = 1.0
    # WEIGHT_B = 0.1

    # NUM_RANKING_ELEMENTS = 4

    # NUM_SIM_RANKINGS = 5000
    # BASE_LIST = [i for i in range(1, NUM_RANKING_ELEMENTS+1)]

    # GROUND_TRUTH_1= np.array([1,2,3,4])
    # GROUND_TRUTH_2= np.array([4,3,2,1])

    # # avoid scientific notation when printing numbers
    # np.set_printoptions(suppress=True)
    # rankings_data_1 = generate_simulated_ranks(BASE_LIST, NUM_RANKING_ELEMENTS, 
    #     WEIGHT_A, WEIGHT_B, NUM_SIM_RANKINGS, data_summary=False, ground_truth=GROUND_TRUTH_1)

    # rankings_data_2 = generate_simulated_ranks(BASE_LIST, NUM_RANKING_ELEMENTS, 
    #     WEIGHT_A, WEIGHT_B, NUM_SIM_RANKINGS, data_summary=False, ground_truth=GROUND_TRUTH_2)

    # # combines two rankings data with two different ground_truths
    # all_rankings_data = np.concatenate((rankings_data_1, rankings_data_2), axis=0)
    ...
    # first_ranking = all_rankings_data[0]
    ...
    # gt_it = 0
    # for gt in tqdm(all_ground_truths):
    #     best_b, best_phi, max_log_like = alt_grad_ascent(all_rankings_data, ground_truth=gt, plot_grads=False)
    #     max_log_likes[gt_it] = max_log_like
    #     best_b_vect[gt_it] = best_b
    ...
    # outfile = 'GT_outputs/SIMULATED_alt_grad.csv'
    ...
    #     for gt in tqdm(all_ground_truths):
    #     #     best_b, best_phi, max_log_like = unweighted_backtrack(data, ground_truth=gt, plot_grads=False)
    #     #     max_log_likes[gt_it] = max_log_like
    #     #     best_b_vect[gt_it] = best_b
    #     #     best_phi_vect[gt_it] = best_phi
    #     #     gt_it += 1
    ...
    #     # outfile = 'GT_outputs/'+dataset+"_out_unweighted_backtrack.csv"
