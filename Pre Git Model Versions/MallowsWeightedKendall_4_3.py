...
def exhaustive_search(data, granularity=101, ground_truth=None, plot_log_likes=True):
    ...
    WEIGHT_A = 1.0
    b = 0.1
    phi_vect = np.linspace(0, 20.0, granularity)
    #b_vect = np.linspace(0, 1.0, granularity)
    
    log_likes = np.zeros_like(phi_vect)
    ...
    best_phi = phi_vect[max_index]

    if plot_log_likes:
        plot_graph(phi_vect, log_likes, "log-likelihood")

    return best_phi, max_likelihood
...
# Current problem is gradient at phi=1 is NOT 0. 
...
def main():
    ...
    # best_b, best_phi, max_log_like = grad_ascent(all_rankings_data, plot_grads=True)

    # print("best_b:", best_b)
    # print("best_phi:", best_phi)
    ...
    for dataset in DATASETS:
        ...
        for gt in tqdm(all_ground_truths):
            ...
            best_b, best_phi, max_log_like = grad_ascent(data, ground_truth=gt, plot_grads=False)
            max_log_likes[gt_it] = max_log_like
            best_b_vect[gt_it] = best_b
        ...
        # outfile = 'GT_outputs/'+dataset+"_out_phi.csv"
